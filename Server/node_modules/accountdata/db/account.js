...
Account.prototype.setAttribute = function(attr, value, callback) {
    this.setField('attributes.' + attr, value, callback);
}

Account.prototype.toJSON = function() {
    // encode everything to JSON besides the hash
    var dictionary = {'email': this.getField('email'),
                      'attributes': {}};
    for (var i = 0; i < exports.attributes.length; i++) {
        var attribute = exports.attributes[i];
        dictionary.attributes[attribute] = this.getAttribute(attribute);
    }
    return dictionary;
}
...
exports.signup = function(username, hash, email, attributes, callback) {
    ...
    var account = {'username': username,
                   ...
                   'attributes': attrs,
                   'puzzleLists': {}};
    dbobject.addOne(account, Account, 'accounts', callback);
}
